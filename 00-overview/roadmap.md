...
| **Reading back a revoked notification permission** | [C8-R24](../10-functional/features/c-insight/c8-notifications.md#acceptance-criteria). A grant the reader later revokes in system settings goes on being reported as a grant: measured on a Galaxy A51, the operating system said denied while the stored row said granted, and the app kept scheduling notifications the platform would drop. The prompt's answer is now recorded when it is given (C8-R23); what is missing is a way to read the current state, which the mobile plugin does not expose. Android could answer synchronously; iOS cannot, and a read that answers on one platform only would leave the reconcile inert exactly where it is needed. Out of v2.0 because it needs a plugin capability on two platforms rather than a fix. |
